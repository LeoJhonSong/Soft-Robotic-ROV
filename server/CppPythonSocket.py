...
class Server():
    ...
    def __receive_value(self, conn, buf_length, decode=True):
        buf = b''
        while buf_length:
            new_buf = conn.recv(buf_length)
            # if not new_buf: return None  # Comment this to make it non-blocking
            buf += new_buf
            buf_length -= len(new_buf)
        if decode:
            return buf.decode()
        else:
            return buf
