...
int main(int argc, const char* argv[]) {
    ...
    std::shared_ptr<torch::jit::script::Module> SSD = torch::jit::load("../models/G_SSD_320.pt");
    SSD->to(at::kCUDA);
    ...
    while(capture.isOpened()){
        ...
        clock_t t2 = clock();
        SSD_input.push_back(img_tensor);
        SSD_output = SSD->forward(SSD_input).toTuple()->elements();
        cudaDeviceSynchronize();
        SSD_input.pop_back();
        clock_t t4 = clock();
        fake_B = SSD_output.at(0).toTensor();
        loc = SSD_output.at(1).toTensor().to(torch::kCPU);
        conf = SSD_output.at(2).toTensor().to(torch::kCPU);
        ota_feature = SSD_output.at(3).toTensor().to(torch::kCPU);
        clock_t t5 = clock();
        ...
        std::cout << "total: " << (t7 - t1) * 1.0 / CLOCKS_PER_SEC * 1000
                  ...
                  <<  ", G_SSD: "  << (t4 - t2) * 1.0 / CLOCKS_PER_SEC * 1000
