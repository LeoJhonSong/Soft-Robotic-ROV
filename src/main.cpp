...
int main(int argc, const char* argv[]) {
    ...
    while(capture.isOpened()){
        ...
        SSD_input.pop_back();
        clock_t t4 = clock();
        loc = SSD_output.at(0).toTensor();
        conf = SSD_output.at(1).toTensor();
        ota_feature = SSD_output.at(2).toTensor();
//        std::cout << fake_B.sizes() << loc.sizes() << conf.sizes() << ota_feature.sizes() << std::endl;
        detections = Detect.detect(loc, conf, conf_thresh);
        clock_t t5 = clock();
        img_vis = tensor2im(fake_B);
        Detect.visualization(img_vis, detections);
        clock_t t6 = clock();
        std::cout << "total: " << (t6 - t1) * 1.0 / CLOCKS_PER_SEC * 1000
                  <<  ", netG: " << (t3 - t2) * 1.0 / CLOCKS_PER_SEC * 1000
                  <<  ", SSD: "  << (t4 - t3) * 1.0 / CLOCKS_PER_SEC * 1000
                  <<  ", detection: "  << (t5 - t4) * 1.0 / CLOCKS_PER_SEC * 1000
                  <<  ", vis: "  << (t6 - t5) * 1.0 / CLOCKS_PER_SEC * 1000 << std::endl;
