...
|K|RUAS滤波参数|**100**|
|R|RUAS滤波参数|**50**|
|NET_PHASE|神经网络模型选择|**0**: 跳过/**1**: 仅netG图像恢复/**2**: netG图像恢复+目标检测/**3**: 仅目标检测|
|STREAM|视频流来源|**-2**: 从指定网络摄像头读取/**-1**: 从视频读取/**0**: 从电脑摄像头读取|
...
|TRACK|是否从候选项中选出一个目标跟踪|**true**/**false**|
...
程序以`#`为帧头, 发送**标记相当于目标**的位置. 坐标正方向如下图所示.
